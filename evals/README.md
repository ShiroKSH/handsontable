...
Per-change run (scope to the touched source files; ALWAYS scope — never the
whole tree):

```bash
cd handsontable
npm run build:styles   # once per clone — two unit contract tests read styles/
BABEL_ENV=commonjs npx env-cmd -f ../hot.config.js   npx stryker run --mutate src/helpers/errors.ts
```
