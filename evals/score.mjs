...
/**
 * Report whether the mutation layer is usable. StrykerJS is a new dependency
 * and needs the minimal-dependency team discussion first, so until
 * `@stryker-mutator/core` resolves the field stays a stub.
 *
...
export function getMutationStatus(resolveModule = createRequire(import.meta.url).resolve) {
  try {
    ...
    return {
      available: true,
      reason: 'stryker resolved — per-change mutation runner is the dependency-gated half, not wired here',
    };
  } catch {
    // Not installed — the expected state until the team signs off on the dependency.
    return { available: false, reason: 'stryker pending team sign-off' };
  }
}
...
export async function scoreTestFile(filePath, options = {}) {
  ...
  return { file: filePath, ...scoreTestSource(src, { diff }) };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (files.length !== 1) {
    console.error('Usage: node evals/score.mjs <test-file> [--diff <diff-file>]');
    process.exitCode = 2;
  } else {
    console.log(JSON.stringify(await scoreTestFile(files[0], { diffPath }), null, 2));
  }
}
