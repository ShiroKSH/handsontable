...
`cd tests && npx playwright test --project=e2e-chromium e2e/<your-spec>.spec.ts`.
The Stop hook and pre-push do exactly this automatically (session-touched /
branch-touched specs). The **full** E2E suite belongs to CI: the PR pipeline
today (a determined subset once impact-selection lands), and the full matrix
nightly on develop. `npm test` in `tests/` locally = wasted minutes, never
required evidence.
