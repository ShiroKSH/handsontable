...
| Agent-time (Claude Code) | `Stop` (turn end) | new-Jasmine check + the touched Playwright specs | a **new** `*.spec.js`; a **failing** touched spec |
...
| **pre-push** (lefthook) | `scripts/pre-push.mjs` | presence gate (block) → eslint on changed → test-weakening detector (warn) → changed Playwright specs | missing test; lint errors; a failing spec |
